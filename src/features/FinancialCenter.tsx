...
export default function FinancialCenter({onClose}:{onClose:()=>void}){
 ...
 const expenses=useMemo(()=>{
   try{
     ...
     return tx.filter(x=>x.status!=="failed" && x.type!=="deposit").map(x=>({
       amount:Number(x.convertedAmount ?? x.amount)||0,
       date:String(x.createdAt||new Date().toISOString()),
       category:txCategory(x),
       source:x.source==="exchange"?"exchange":x.type==="service"?"bill":x.type==="transfer"?"transfer":"other"
     }));
   }catch{return[]}
 },[phone]);
 ...
 const submit=()=>{
   ...
   const min=Math.max(0,Number(draft.minAmount)||0);
   const max=Math.max(min,Number(draft.maxAmount)||0);
 ...
 return <div dir="rtl" className="anp-finance-page"><header><button className="back-btn" onClick={onClose}>‹</button><div><h2>مرکز مالی</h2><p>تحلیل درآمد، هزینه و وضعیت مالی</p></div></header><main>
  <section className={`anp-finance-status ${insight.status}`}><div><b>{statusText}</b><div className="muted">{statusHint}</div></div><span>نسبت هزینه به درآمد: {fa(insight.expenseRatio*100)}٪</span></section>
